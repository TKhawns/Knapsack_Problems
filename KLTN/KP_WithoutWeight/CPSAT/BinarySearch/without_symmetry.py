    ...
    if status == cp_model.OPTIMAL:
        ...
        for i, xs_val in enumerate(xs):
            if solver.value(xs_val):
                is_rotated = solver.value(r[i])
                if is_rotated:
                    width, height = rectangles[i][1], rectangles[i][0]
                else:
                    width, height = rectangles[i][0], rectangles[i][1]
                selected_rectangles.append((width, height, profits[i]))
                pos.append([solver.value(x[i]), solver.value(y[i])])
        profits = sum(rec[2] for rec in selected_rectangles)
        print(profits, pos)
        display_solution((W,H), selected_rectangles, pos)
