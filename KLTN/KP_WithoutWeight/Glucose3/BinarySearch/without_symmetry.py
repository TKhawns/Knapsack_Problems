            ...
            with open(file_path, 'r') as file:
                ...
                while (lower_bound + 1 < upper_bound):
                    if (time.time() - started_time >= 600):
                        isExport = "TIMEOUT"
                        export_csv(file_name, "glucose_non_symmetry", time.time() - started_time, "TIMEOUT", 0, 0, 0, 0, 0)
                        break
                ...
                if (isExport != "TIMEOUT"):
                    if (prev_sat[0]) == "SAT":
                        export_csv(file_name, "glucose_non_symmetry", ended_time - started_time, "SAT", prev_sat[2], prev_sat[3], prev_sat[4], prev_sat[5], prev_sat[1])
                    else: export_csv(file_name, "glucose_non_symmetry", ended_time - started_time, "UNSAT", 0, 0, 0, 0, 0 )
