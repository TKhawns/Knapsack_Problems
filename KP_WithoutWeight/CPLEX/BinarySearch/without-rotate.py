...
def display_solution(strip, rectangles, pos_circuits):
    ...
    if len(pos_circuits) > 0:
        for i in range(len(rectangles)):
            width, height, _ = rectangles[i]  # Lấy width và height từ rectangle
            rect = plt.Rectangle(
                (pos_circuits[i][0], pos_circuits[i][1]),  # position (x,y)
                width,  # width
                height,  # height
                edgecolor="#333"
            )            
...
def cplex_constraints(rectangles, W, H, profits, mid):
    ...
    if solution:
        ...
        display_solution((W,H), selected_rectangles, positions)
            ...
            with open(file_path, 'r') as file:
                ...
                while (lower_bound + 1 < upper_bound):
                    if (time.time() - started_time >= 600):
                        isExport = "TIMEOUT"
                        if (prev_sat != []):
                            export_csv(file_name, "cplex_non_rotate_studio", time.time() - started_time, "TIMEOUT", prev_sat[3], prev_sat[4], prev_sat[2])
                            break
                        export_csv(file_name, "cplex_non_rotate_studio", time.time() - started_time, "TIMEOUT", 0, 0, 0)
                        break
                ...
                if (isExport != "TIMEOUT"):
                    if (prev_sat[0]) == "SAT":
                        export_csv(file_name, "cplex_non_rotate_studio", ended_time - started_time, "SAT", prev_sat[3], prev_sat[4], prev_sat[2])
                    else: export_csv(file_name, "cplex_non_rotate_studio", ended_time - started_time, "UNSAT", 0, 0, 0)
