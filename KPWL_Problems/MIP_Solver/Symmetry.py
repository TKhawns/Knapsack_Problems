...
def MIP_OPP(rectangles, W, H):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            # If rectangles can't fit horizontally
            if wi + wj > W:
                # Force vertical arrangement
                solver.Add(y[i] + hi <= y[j] + H * no_overlap_3)
                solver.Add(y[j] + hj <= y[i] + H * no_overlap_4)
            
            # If rectangles can't fit vertically
            if hi + hj > H:
                # Force horizontal arrangement
                solver.Add(x[i] + wi <= x[j] + W * no_overlap_1)
                solver.Add(x[j] + wj <= x[i] + W * no_overlap_2)
