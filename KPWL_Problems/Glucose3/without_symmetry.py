...
def export_csv(problem_name, method_name, time, result, num_var, num_clause,num_var_solver,num_clause_solver,  max_profit, weights):
    global id_counter
    id_counter += 1
    result_dict = {
        "No": id_counter,
...
def max_profit_solution():
    ...
    for file_name in os.listdir(folder_path):
        ...
        if os.path.isfile(file_path) and file_name.endswith('.txt') and file_name == "r_20_l.txt":
            ...
            with open(file_path, 'r') as file:
                ...
                while (lower_bound + 1 < upper_bound):
                    if (time.time() - started_time >= 600):
                        isExport = "TIMEOUT"
                        export_csv(file_name, "glucose_non_symmetry", time.time() - started_time, "TIMEOUT", 0, 0, 0, 0, 0, 0)
                        break
                ...
                if (isExport != "TIMEOUT"):
                    if (prev_sat[0]) == "SAT":
                        export_csv(file_name, "glucose_non_symmetry", ended_time - started_time, "SAT", prev_sat[2], prev_sat[3], prev_sat[4], prev_sat[5], prev_sat[1], prev_sat[6])
                    else: export_csv(file_name, "glucose_non_symmetry", ended_time - started_time, "UNSAT", 0, 0, 0, 0, 0, 0)
