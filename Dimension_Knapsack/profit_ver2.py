...
def opp_solution(rectangles, strip, length, isCount):
    cnf = CNF()
    variables = {}
    counter = 1
    ...
    #  Number of variables and clauses must be called 1 time.
    if isCount:
    ...
    with open('dataset.txt', 'r') as file:
        ...
        for i in range(0, len_file, 4):
            ...
            for i in range(0, len(result)):
                ...
                #  status = ["isSAT", number of variables, number of clauses]
                status = opp_solution(result, strip, num_items, isCount)
                if (len(result) == 0):
            ...
            solve_by_pysat(str(num_items), str(elap_time), solution, status[1], status[2], "Glucose3")
